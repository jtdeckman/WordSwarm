 ...
 * @param word the word to look up in the dictionary.
 * @param definitionBodyFontSize the font size (in pixels) to use for the definition's text.
 * @param useHyperlinks if true, the words will be converted to hyperlinks using the app's custom-scheme (touchableWordsURLScheme)
 */
- (NSString *)definitionAsHTMLFor:(NSString *)word withStyleSheet:(NSString *)css useHyperlinks:(BOOL)useHyperlinks;
